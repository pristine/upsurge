...
import { db } from "../../../db";
import { type RedeemedReward, type Reward } from "@prisma/client";
import Embed from "../../../embed";

const formatRewards = (
  rewards: Array<RedeemedReward & { Reward: Reward }>,
  skipCount?: number,
): string => {
  const formatted = rewards.map((reward, index) => {
    return `${index + (skipCount ?? 0) + 1}) **${
      reward.Reward.nickname
    }** - ${reward.createdAt.toLocaleString("en-US", {
      timeZone: "America/New_York",
      timeZoneName: "short",
    })} - ${reward.processed ? "Processed" : "Not Processed"}`;
  });
...
export default new SlashCommand(
  {
    name: "redeemed",
    description: "List redeemed rewards",
    options: [],
  },
  {
    ...
    run: async (interaction) => {
      ...
      try {
        ...
        const baseEmbed = new Embed()
          .setTitle(
            `Redeemed Rewards (Page 1/${Math.ceil(count / takeAmount)})`,
          )
              ...
              collector.on("collect", (reaction, user) => {
                ...
                void reaction.users
                  ...
                  .then(async () => {
                    ...
                    const rewards = await db.redeemedReward.findMany({
                      where: {
                        serviceId: guild.id,
                        serviceType: "discord",
                      },
                    ...
                    const embed = new Embed()
                      .setTitle(
                        `Redeemed Rewards (Page ${currentPage + 1}/${Math.ceil(
                          count / takeAmount,
                        )})`,
                      )
                      .setDescription(
